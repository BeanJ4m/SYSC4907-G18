...
class TabTransformer(nn.Module):
    ...
    def forward(self, x):
        # Assert input matches declared number of features 
        ...
        # Assert we have correct number of output classes
...
def save_metrics_to_csv(csv_path, accuracy, precision, recall, f1, epoch, batch_size, lr):
    log_file = "Data/training_metrics_log.csv"
...
def train(csv_path, target_column="label", epochs=10, batch_size=32, lr=1e-4):
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1}/{epochs} | Loss={avg_loss:.4f} "
            f"| Accuracy={accuracy*100:.2f}% | P={precision:.4f} R={recall:.4f} F1={f1:.4f}")
...
if __name__ == "__main__":
    model = train("Data/2_Dataset_5_Attack_30_One_Attack_Type.csv",target_column="Attack_type",epochs=10,batch_size=128,lr=1e-4)
    torch.save(model.state_dict(), "Data/tabtransformer_model.pt")
