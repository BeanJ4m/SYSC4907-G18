...
from data_preparation import Dataloaders, ROUNDS
from torch.utils.data import ConcatDataset

from performance_testing import performance_testing
...
def prepare_dataloaders(n_splits, batch_size):

    if n_splits > ROUNDS:
        raise ValueError(f"Requested n_splits={n_splits} but data_preparation only created {ROUNDS} rounds.")

    train_loaders = []
    all_labels = []

    # Build one combined DataLoader per round
    for round_idx in range(1, n_splits + 1):
        # Dataloaders[round_idx] is a list of client DataLoaders
        client_loaders = Dataloaders[round_idx]

        # Combine all client subsets into one big dataset for this round
        datasets = [dl.dataset for dl in client_loaders]
        combined_dataset = ConcatDataset(datasets)

        # Use the TabTransformer batch_size here, not the FL batch size
        loader = DataLoader(combined_dataset, batch_size=batch_size, shuffle=False)
        train_loaders.append(loader)

        # Collect labels for class-weight computation
        for dl in client_loaders:
            for _, y_batch in dl:
                all_labels.extend(y_batch.numpy())

    # Build test_loader from the FL-style test loader
    base_test_loader = Dataloaders["Test"]
...
def train(
    ...
    for train_loader_idx, train_loader in enumerate(train_loaders, start=1):
        ...
        for epoch in range(1, epochs_per_round + 1):
            ...
            print(f"[Split {train_loader_idx}/{n_splits}] Epoch {epoch}/{epochs_per_round} | Loss={avg_loss:.4f}")
